...
import { calculateSingleSkillDamage } from "@/domain/combat/combat-engine";
...
import { createBaseAttackPowerSnapshot, createCombatStatSnapshot, createFinalAttackPowerSnapshot, createPureAttackPowerSnapshot, createWeaponAttackSnapshot } from "@/domain/combat/combat-stat-snapshot";
...
const gauntletEnhancementLevels = Array.from({ length: 26 }, (_, index) => 25 - index);
...
const gemIconCdn: Record<string, Record<number, string>> = {
  겁화: { 10: "https://cdn-lostark.game.onstove.com/efui_iconatlas/use/use_12_105.png", 9: "https://cdn-lostark.game.onstove.com/efui_iconatlas/use/use_12_104.png", 8: "https://cdn-lostark.game.onstove.com/efui_iconatlas/use/use_12_103.png", 7: "https://cdn-lostark.game.onstove.com/efui_iconatlas/use/use_12_102.png", 6: "https://cdn-lostark.game.onstove.com/efui_iconatlas/use/use_12_101.png" },
  작열: { 10: "https://cdn-lostark.game.onstove.com/efui_iconatlas/use/use_12_115.png", 9: "https://cdn-lostark.game.onstove.com/efui_iconatlas/use/use_12_114.png", 8: "https://cdn-lostark.game.onstove.com/efui_iconatlas/use/use_12_113.png", 7: "https://cdn-lostark.game.onstove.com/efui_iconatlas/use/use_12_112.png", 6: "https://cdn-lostark.game.onstove.com/efui_iconatlas/use/use_12_111.png" },
};
const radiantGemIconCdn: Record<number, string> = { 10: "https://cdn-lostark.game.onstove.com/efui_iconatlas/use/use_13_55.png", 9: "https://cdn-lostark.game.onstove.com/efui_iconatlas/use/use_13_54.png", 8: "https://cdn-lostark.game.onstove.com/efui_iconatlas/use/use_13_53.png", 7: "https://cdn-lostark.game.onstove.com/efui_iconatlas/use/use_13_52.png", 6: "https://cdn-lostark.game.onstove.com/efui_iconatlas/use/use_13_51.png" };
function gemIconFor(type: string, level: number) { return gemIconCdn[type]?.[level] ?? null; }
function isRadiantGem(gem: GemProfile) { return gem.name.includes("광휘"); }
function gemDisplayIcon(gem: GemProfile, type = gem.type, level = gem.level ?? 10) { if (isRadiantGem(gem)) return radiantGemIconCdn[level] ?? gem.icon; if (gem.name.includes("멸화")) return melhaGemIcon.src; if (gem.name.includes("홍염")) return hongyeomGemIcon.src; return gemIconFor(type, level) ?? gem.icon; }
function normalizeGem(gem: GemProfile): GemProfile { const level = Math.max(6, Math.min(10, gem.level ?? 10)); const cooldown = /재사용 대기시간|쿨타임/.test(gem.effect ?? ""); const type = gem.type === "작열" || gem.type === "홍염" || (isRadiantGem(gem) && cooldown) ? "작열" : "겁화"; const normalized = { ...gem, type, level }; return { ...normalized, icon: gemDisplayIcon(normalized) }; }
...
function isArkPassivePointOption(option: string) { return /^(진화|깨달음|도약)\s*\+?\s*\d+/.test(option.trim()); }
...
function GearEditor({ item, onChange }: { item: EquipmentProfile; onChange: (patch: Partial<EquipmentProfile>) => void }) {
  ...
  const selectableEnhancements = isArmGauntlet ? gauntletEnhancementLevels : enhancementLevels;
...
function AccessoryEditor({ item, onChange }: { item: EquipmentProfile; onChange: (patch: Partial<EquipmentProfile>) => void }) {
  const options = Array.from({ length: 3 }, (_, index) => item.options.filter((option) => !isArkPassivePointOption(option))[index] ?? "없음");
  ...
  const commitStat = () => {
    ...
    const normalized = Number.isFinite(value) && value >= range.min && value <= range.max ? value : range.min;
    const next = String(normalized);
    setDraftStat(next);
    onChange({ baseStats: [next] });
  };
  return <article className="accessory-editor"><div className="quality-art"><Artwork icon={item.icon} label="◇" /><span className={accessoryStatTone(item)}>{accessoryStatPercent(item)}</span></div><div className="accessory-meta"><select aria-label={`${item.slot} 등급`} value={item.simulationGrade === "T4 전율" ? "고대" : item.simulationGrade} onChange={(event) => onChange({ simulationGrade: event.target.value as EquipmentProfile["simulationGrade"] })}><option value="고대">고대</option><option value="유물">유물</option></select><input aria-label={`${item.slot} 힘민지`} type="number" inputMode="numeric" value={draftStat} onChange={(event) => setDraftStat(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") { event.preventDefault(); commitStat(); } }} /></div><div className="accessory-option-list">{options.map((option, index) => { const choices = optionChoices(item.slot, option, accessoryOptions); return <select aria-label={`${item.slot} 옵션 ${index + 1}`} value={option} onChange={(event) => { const next = [...options]; next[index] = event.target.value; onChange({ options: next }); }} key={index}>{choices.map((value) => <option value={value} key={value}>{value}</option>)}</select>; })}</div></article>;
}
...
function GemBoard({ gems, skills, onChange, onAdd, onRemove, onBulkLevel, message }: { gems: GemProfile[]; skills: SkillProfile[]; onChange: (id: string, patch: Partial<GemProfile>) => void; onAdd: () => void; onRemove: (id: string) => void; onBulkLevel: (level: number) => void; message: string }) { return <section className="gem-board"><div className="gem-board-heading"><div><h2>보석</h2></div><div className="gem-bulk-controls"><strong>일괄 변경</strong>{[10, 9, 8, 7].map((level) => <button type="button" onClick={() => onBulkLevel(level)} key={level}>{level}겁작</button>)}</div></div>{message ? <p className="validation-message" role="alert">{message}</p> : null}<div className="gem-board-list">{gems.map((gem) => { const level = gem.level ?? 10; const icon = gemDisplayIcon(gem); return <article key={gem.id}><div className="gem-board-art"><Artwork icon={icon} label="◆" /><b>{level}</b></div><select aria-label={`${gem.skill || "미지정"} 보석 종류`} value={gem.type} onChange={(event) => onChange(gem.id, { type: event.target.value })}>{gemTypes.map((type) => <option key={type}>{type}</option>)}</select><select aria-label={`${gem.skill || "미지정"} 보석 레벨`} value={level} onChange={(event) => onChange(gem.id, { level: Number(event.target.value) })}>{gemLevels.map((optionLevel) => <option value={optionLevel} key={optionLevel}>{optionLevel}</option>)}</select><select aria-label="보석 적용 스킬" value={gem.skill ?? ""} onChange={(event) => onChange(gem.id, { skill: event.target.value })}><option value="">스킬 선택</option>{skills.map((skill) => <option value={skill.name} key={skill.id}>{skill.name}</option>)}</select><button type="button" className="gem-remove-button" onClick={() => onRemove(gem.id)} aria-label={`${gem.skill || "보석"} 제거`}>×</button></article>; })}{gems.length < 11 ? <button type="button" className="gem-board-add" onClick={onAdd}>+ 보석 추가</button> : null}</div></section>; }
function SkillEditor({ skill, gems, combatBase, onChange, onGemChange, onRemoveGem, onAddGem }: { skill: SkillProfile; gems: GemProfile[]; combatBase: { attackPower: number; criticalRate: number }; onChange: (patch: Partial<SkillProfile>) => void; onGemChange: (id: string, patch: Partial<GemProfile>) => void; onRemoveGem: (id: string) => void; onAddGem: (type: "겁화" | "작열") => void }) { const tripods = Array.from({ length: 3 }, (_, index) => skill.tripods[index] ?? { name: "없음", level: null }); const catalog = GLAVIER_SKILL_TRIPODS[skill.name as keyof typeof GLAVIER_SKILL_TRIPODS]; const details = GLAVIER_SKILL_TRIPOD_DETAILS[skill.name] ?? []; const selectedTripodNames = tripods.map((tripod) => tripod.name); const cooldown = resolveGlavierSkillCooldown({ skillName: skill.name, selectedTripodNames }); const calculation = calculateSingleSkillDamage({ base: { primaryStat: 6, weaponAttack: combatBase.attackPower ** 2, criticalRate: combatBase.criticalRate }, effects: [], skill: { name: skill.name, level: skill.level, selectedTripodNames } }); const orderedGems = [...gems].sort((a, b) => (a.type === "겁화" ? 0 : 1) - (b.type === "겁화" ? 0 : 1)); return <article className="skill-card skill-editor"><Artwork icon={skill.icon} label="✦" /><strong className="skill-name">{skill.name}</strong><select className="skill-level-select" aria-label={`${skill.name} 레벨`} value={skill.level} onChange={(event) => onChange({ level: Number(event.target.value) })}>{skillLevels.map((level) => <option value={level} key={level}>Lv.{level}</option>)}</select><div className="skill-tripod-selects">{tripods.map((tripod, index) => { const options = [...new Set([tripod.name, ...(catalog?.[index] ?? [])])]; const detail = details.find((item) => item.tier === index + 1 && item.name === tripod.name); return <div className="skill-tripod-tooltip" data-tooltip={detail?.description ?? tripod.name} key={`${tripod.name}-${index}`}><select aria-label={`${skill.name} 트라이포드 ${index + 1}`} value={tripod.name} onChange={(event) => onChange({ tripods: tripods.map((item, itemIndex) => itemIndex === index ? { ...item, name: event.target.value, level: event.target.value === "없음" ? null : item.level } : item) })}>{options.map((option) => <option value={option} key={option}>{option}{option === tripod.name && tripod.level ? ` Lv.${tripod.level}` : ""}</option>)}</select></div>; })}</div><div className="skill-gem-inline">{gemTypes.map((type) => { const gem = orderedGems.find((candidate) => candidate.type === type); return gem ? <div className="skill-gem-slot" key={gem.id}><Artwork icon={gemDisplayIcon(gem)} label="◆" /><select aria-label={`${type} 보석 종류`} value={gem.type} onChange={(event) => onGemChange(gem.id, { type: event.target.value })}>{gemTypes.map((gemType) => <option key={gemType}>{gemType}</option>)}</select><select aria-label={`${type} 보석 레벨`} value={gem.level ?? 10} onChange={(event) => onGemChange(gem.id, { level: Number(event.target.value) })}>{gemLevels.map((level) => <option value={level} key={level}>{level}</option>)}</select><button type="button" className="skill-gem-remove" onClick={() => onRemoveGem(gem.id)} aria-label="보석 삭제">×</button></div> : <button type="button" className="skill-gem-add-slot" key={`gem-add-${type}`} onClick={() => onAddGem(type)}>{type} 보석 추가</button>; })}</div><div className="skill-metrics" aria-label={`${skill.name} 전투 데이터`}><div><span>스킬 쿨타임 <b>{cooldown ? `${cooldown.cooldownSeconds.toFixed(1)}초` : "-"}</b></span><span>치명타 확률 <b>{`${(calculation.combat.stages.criticalRate * 100).toFixed(2)}%`}</b></span></div><div><span>최대 대미지 <b>{Math.floor(calculation.maximumCriticalDamage).toLocaleString()}</b></span><span>평균 대미지 <b>{Math.floor(calculation.expectedDamage).toLocaleString()}</b></span></div></div></article>; }
...
function initialAvatarGrades(profile: CharacterProfile) {
  const avatarFor = (slot: (typeof avatarSlots)[number]) => {
    const direct = profile.avatars.find((item) => item.slot.includes(slot) || (slot === "머리" && item.slot.includes("투구")));
    // 상·하의 일체형 아바타는 API에서 상의 한 칸만 내려오는 경우가 있다.
    // 하의 데이터가 없을 때만 상의 등급을 하의에도 적용한다.
    if (slot === "하의" && !direct) return profile.avatars.find((item) => item.slot.includes("상의"));
    return direct;
  };
  return Object.fromEntries(avatarSlots.map((slot) => {
    const avatar = avatarFor(slot);
    return [slot, avatar?.grade === "전설" ? "전설" : avatar ? "영웅" : "없음"];
  })) as Record<(typeof avatarSlots)[number], string>;
}

function InternalGearSnapshotDebug({ gear, character, avatarGrades, stoneEffects, gems }: { gear: EquipmentProfile[]; character: CharacterProfile; avatarGrades: Record<string, string>; stoneEffects: StoneEffect[]; gems: GemProfile[] }) {
  const snapshot = createInternalGearSnapshot(gear);
  const combatStats = createCombatStatSnapshot({ equipment: character.equipment, avatarGrades });
  const enlightenmentLevel = character.arkPassive.points.find((point) => point.name === "깨달음")?.level ?? 0;
  const weaponAttack = createWeaponAttackSnapshot({ equipment: character.equipment, arkGridCores: character.arkGrid.cores, enlightenmentRate: enlightenmentWeaponAttackRate(enlightenmentLevel) });
  const pureAttackPower = createPureAttackPowerSnapshot(combatStats.total, weaponAttack.total);
  const baseAttackPower = createBaseAttackPowerSnapshot({ pureAttackPower: pureAttackPower.total, gauntletFlat: snapshot.baseAttackFlat, gauntletRate: snapshot.baseAttackRate, stoneLevels: stoneEffects.map((effect) => effect.level), gems });
  const finalAttackPower = createFinalAttackPowerSnapshot({ baseAttackPower: baseAttackPower.total, equipment: character.equipment, arkGridEffects: character.arkGrid.effects, arkGridCores: character.arkGrid.cores, engravings: character.engravingDetails, stoneEffects });
  const format = (value: number | null, suffix = "") => value === null ? "미등록" : value.toLocaleString() + suffix;
  return <details className="internal-gear-debug" open>
    <summary>내부 장비 스냅샷 · {snapshot.unresolvedSlots.length ? snapshot.unresolvedSlots.length + "개 미등록" : "검증 완료"}</summary>
    <div className="internal-gear-debug-summary">
      <span>기본 <b>{combatStats.base.toLocaleString()}</b></span><span>장비 <b>{combatStats.gear.toLocaleString()}</b></span><span>악세 <b>{combatStats.accessories.toLocaleString()}</b></span><span>팔찌 <b>{combatStats.bracelet.toLocaleString()}</b></span><span>합산 <b>{combatStats.subtotal.toLocaleString()}</b></span><span>배율 <b>{((combatStats.petRate + combatStats.avatarRate) * 100).toFixed(2)}%</b></span><span>최종 <b>{Math.ceil(combatStats.total).toLocaleString()}</b></span>
      <span>무공 장비 <b>{weaponAttack.gear.toLocaleString()}</b></span><span>무공 악세 <b>{weaponAttack.accessoriesFlat.toLocaleString()}</b></span><span>무공 팔찌 <b>{weaponAttack.bracelet.toLocaleString()}</b></span><span>무공 증가 <b>{(weaponAttack.rateTotal * 100).toFixed(2)}%</b></span><span>최종 무공 <b>{Math.floor(weaponAttack.total).toLocaleString()}</b></span><span>순수 공격력 <b>{pureAttackPower.total.toLocaleString(undefined, { maximumFractionDigits: 2 })}</b></span><span>기본공 완갑 <b>{baseAttackPower.gauntletFlat.toLocaleString()}</b></span><span>기본공 완갑% <b>{(baseAttackPower.gauntletRate * 100).toFixed(2)}%</b></span><span>기본공 스톤 <b>{(baseAttackPower.stoneRate * 100).toFixed(2)}%</b></span><span>기본공 보석 <b>{(baseAttackPower.gemRate * 100).toFixed(2)}%</b></span><span>기본 공격력 <b>{baseAttackPower.total.toLocaleString(undefined, { maximumFractionDigits: 2 })}</b></span><span>공격력 악세 <b>{finalAttackPower.accessoriesFlat.toLocaleString()}</b></span><span>공격력 증가 <b>{(finalAttackPower.rateTotal * 100).toFixed(2)}%</b></span><span>최종 공격력 <b>{finalAttackPower.total.toLocaleString(undefined, { maximumFractionDigits: 2 })}</b></span>
      <span>기본 공격력 <b>{(snapshot.baseAttackRate * 100).toFixed(2)}%</b></span>
    </div>
    <div className="internal-gear-debug-rows">
      {snapshot.rows.map((row) => <div className={row.status === "resolved" ? "" : "unresolved"} key={row.slot}>
...
export default function Home() {
  const [menu, setMenu] = useState<MainMenu>("simulation"); const [tab, setTab] = useState<SimulationTab>("전체"); const [apiKey, setApiKey] = useState(""); const [rememberApiKey, setRememberApiKey] = useState(false); const [hasSavedApiKey, setHasSavedApiKey] = useState(false); const [characterName, setCharacterName] = useState(""); const [character, setCharacter] = useState<CharacterProfile | null>(null); const [message, setMessage] = useState("API 설정에서 API 키를 입력한 뒤 캐릭터를 조회하세요."); const [searching, setSearching] = useState(false); const [cycle, setCycle] = useState<string[]>([]); const [cycleSkill, setCycleSkill] = useState(""); const [skillToAdd, setSkillToAdd] = useState(""); const [savedSettings, setSavedSettings] = useState<SavedSetting[]>([]); const [gems, setGems] = useState<GemProfile[]>([]); const [gemMessage, setGemMessage] = useState(""); const [stoneEffects, setStoneEffects] = useState<StoneEffect[]>([]); const [avatarGrades, setAvatarGrades] = useState<Record<string, string>>({});
  function applyProfile(profile: CharacterProfile) { const cleanProfile = { ...profile, gems: profile.gems.map(normalizeGem), arkPassive: { ...profile.arkPassive, evolution: profile.arkPassive.evolution.filter((effect): effect is ArkEffectProfile => Boolean(effect)), enlightenment: profile.arkPassive.enlightenment.filter((effect): effect is ArkEffectProfile => Boolean(effect)).map((effect, index) => ({ ...effect, id: `enlightenment-api-${index}-${effect.id}` })), leap: profile.arkPassive.leap.filter((effect): effect is ArkEffectProfile => Boolean(effect)) } }; setCharacter(cleanProfile); setCharacterName(cleanProfile.name); setGems(cleanProfile.gems); setStoneEffects(initialStoneEffects(cleanProfile)); setAvatarGrades(initialAvatarGrades(cleanProfile)); setGemMessage(""); }
  ...
  useEffect(() => { try { const saved = localStorage.getItem(API_KEY_STORAGE_KEY); if (saved) { setApiKey(saved); setRememberApiKey(true); setHasSavedApiKey(true); } } catch { /* 브라우저 저장소를 사용할 수 없으면 저장 기능만 비활성화한다. */ } }, []);
  useEffect(() => {
    if (tab !== "기본 장비") return;
  ...
  function updateApiKey(value: string) { setApiKey(value); if (!rememberApiKey) return; try { if (value.trim()) { localStorage.setItem(API_KEY_STORAGE_KEY, value.trim()); setHasSavedApiKey(true); } else { localStorage.removeItem(API_KEY_STORAGE_KEY); setHasSavedApiKey(false); } } catch { setMessage("브라우저 저장소에 API 키를 저장하지 못했습니다."); } }
  function toggleApiKeyRemember(checked: boolean) { if (checked && !apiKey.trim()) { setMessage("저장할 API 키를 먼저 입력해주세요."); return; } try { if (checked) { localStorage.setItem(API_KEY_STORAGE_KEY, apiKey.trim()); setHasSavedApiKey(true); setMessage("이 브라우저에 API 키를 저장했습니다."); } else { localStorage.removeItem(API_KEY_STORAGE_KEY); setHasSavedApiKey(false); setMessage("저장된 API 키를 삭제했습니다."); } setRememberApiKey(checked); } catch { setMessage("브라우저 저장소를 사용할 수 없습니다."); } }
  function removeSavedApiKey() { try { localStorage.removeItem(API_KEY_STORAGE_KEY); setApiKey(""); setRememberApiKey(false); setHasSavedApiKey(false); setMessage("저장된 API 키를 삭제했습니다."); } catch { setMessage("브라우저 저장소를 사용할 수 없습니다."); } }
  ...
  function addGem(skillName = cycleSkills[0]?.name ?? "", type: "겁화" | "작열" = "겁화") { if (gems.length >= 11) { setGemMessage("보석은 최대 11개까지만 선택할 수 있습니다."); return; } setGems((current) => [...current, { id: `custom-${crypto.randomUUID()}`, name: `${type} 보석`, type, level: 10, grade: "고대", icon: null, skill: skillName, effect: null }]); setGemMessage(""); }
  function updateGem(id: string, patch: Partial<GemProfile>) { setGems((current) => current.map((gem) => gem.id === id ? normalizeGem({ ...gem, ...patch }) : gem)); }
  function updateAllGemLevels(level: number) { setGems((current) => current.map((gem) => normalizeGem({ ...gem, level }))); setGemMessage(""); }
  function updateSkill(id: string, patch: Partial<SkillProfile>) { setCharacter((current) => current ? { ...current, skills: current.skills.map((skill) => skill.id === id ? { ...skill, ...patch } : skill) } : current); }
  function addSkillToList() { if (!skillToAdd) return; updateSkill(skillToAdd, { level: 2 }); setSkillToAdd(""); }
...
return <main className="shell simulator-shell"><header className="topbar"><div className="brand"><span className="brand-mark">G</span><div><strong>GLAVIER</strong><small>DPS SIMULATOR</small></div></div><nav className="main-menu">{[["simulation", "시뮬레이션"], ["comparison", "세팅 비교"], ["api", "API 설정"]].map(([id, label]) => <button type="button" className={menu === id ? "active" : ""} onClick={() => setMenu(id as MainMenu)} key={id}>{label}</button>)}<div className="header-character-search"><form onSubmit={search}><input aria-label="캐릭터명" value={characterName} onChange={(event) => setCharacterName(event.target.value)} placeholder="캐릭터명 입력" maxLength={24} /><button type="submit" disabled={searching}>{searching ? "조회 중" : "검색"}</button></form></div></nav></header>
{menu === "api" ? <section className="workspace api-workspace"><div className="workspace-title"><span>03</span><div><h1>API 설정</h1><p>선택하면 API 키를 현재 브라우저의 localStorage에 저장해 다음 접속에도 사용합니다.</p></div></div><label className="api-field">Lost Ark API Key<input type="password" value={apiKey} onChange={(event) => updateApiKey(event.target.value)} placeholder="API 키 또는 bearer API 키" autoComplete="off" /></label><div className="api-storage-controls"><label><input type="checkbox" checked={rememberApiKey} onChange={(event) => toggleApiKeyRemember(event.target.checked)} /> 이 브라우저에 API 키 저장</label>{hasSavedApiKey ? <><span>저장됨 · {apiKey.length > 8 ? `${apiKey.slice(0, 4)}••••${apiKey.slice(-4)}` : "••••••••"}</span><button type="button" onClick={removeSavedApiKey}>저장된 키 삭제</button></> : <span>저장하지 않음</span>}</div><p className="api-storage-note">공용 PC에서는 저장하지 마세요. 키는 이 브라우저에서만 사용됩니다.</p><a className="guide-link" href="https://developer-lostark.game.onstove.com/" target="_blank" rel="noreferrer">로스트아크 Open API 키 발급 가이드 ↗</a></section> : null}
...
{menu === "simulation" ? <>
{character ? <section className="workspace simulation-workspace"><div className="profile-strip"><Artwork icon={character.characterImage} label="⚔" /><div><span>{character.server} · {character.className}</span><h1>{character.name}</h1></div><div className="profile-dps"><span>예상 DPS</span><strong>계산 준비 중</strong></div><strong>아이템 레벨 {character.level}</strong><button type="button" onClick={saveSetting}>현재 세팅 저장</button></div><InternalGearSnapshotDebug gear={gear} character={character} avatarGrades={avatarGrades} stoneEffects={stoneEffects} gems={gems} /><nav className="sim-tabs" aria-label="시뮬레이션 탭">{simTabs.map((item) => <button type="button" className={tab === item ? "active" : ""} onClick={() => setTab(item)} key={item}>{item}</button>)}</nav><div className="sim-content">
...
{tab === "스킬 & 전투 사이클" ? <div className="skills-cycle"><section className="skill-list-section"><div className="section-heading"><div><h2>스킬</h2></div><div className="skill-bulk-controls"><strong>일괄 변경</strong>{[10, 9, 8, 7].map((level) => <button type="button" onClick={() => updateAllGemLevels(level)} key={level}>{level}겁작</button>)}</div></div><div className="skills-list">{visibleSkills.map((skill) => <SkillEditor skill={skill} gems={gems.filter((gem) => gem.skill === skill.name)} combatBase={{ attackPower: Number(character.combat.attackPower.replaceAll(",", "")) || 0, criticalRate: (Number(character.combat.criticalChance.replace("%", "")) || 0) / 100 }} onChange={(patch) => updateSkill(skill.id, patch)} onGemChange={updateGem} onRemoveGem={(id) => { setGems((current) => current.filter((gem) => gem.id !== id)); }} onAddGem={(type) => addGem(skill.name, type)} key={skill.id} />)}</div>{addableSkills.length ? <div className="skill-add"><select value={skillToAdd} onChange={(event) => setSkillToAdd(event.target.value)}><option value="">추가할 스킬 선택</option>{addableSkills.map((skill) => <option value={skill.id} key={skill.id}>{skill.name}</option>)}</select><button type="button" onClick={addSkillToList}>스킬 추가</button></div> : null}</section><section className="cycle-builder"><div className="section-heading"><div><h2>전투 사이클 구성</h2><p>스킬을 추가한 뒤 위·아래로 이동해 사용 순서를 만드세요.</p></div><span>{cycle.length}개</span></div><div className="cycle-add"><select value={cycleSkill} onChange={(event) => setCycleSkill(event.target.value)}><option value="">스킬 선택</option>{cycleSkills.map((skill) => <option value={skill.name} key={skill.id}>{skill.name}</option>)}</select><button type="button" onClick={() => cycleSkill && setCycle((value) => [...value, cycleSkill])}>추가</button></div>{cycle.length ? <ol className="cycle-list">{cycle.map((skill, index) => <li key={`${skill}-${index}`}><b>{index + 1}</b><span>{skill}</span><button type="button" onClick={() => setCycle((value) => { const next = [...value]; if (index > 0) [next[index - 1], next[index]] = [next[index], next[index - 1]]; return next; })}>↑</button><button type="button" onClick={() => setCycle((value) => { const next = [...value]; if (index < next.length - 1) [next[index + 1], next[index]] = [next[index], next[index + 1]]; return next; })}>↓</button><button type="button" onClick={() => setCycle((value) => value.filter((_, itemIndex) => itemIndex !== index))}>삭제</button></li>)}</ol> : <p className="empty-copy">전투 사이클에 사용할 스킬을 추가하세요.</p>}</section></div> : null}
